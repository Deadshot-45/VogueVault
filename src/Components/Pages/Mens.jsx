...
const Mens = () => {
  // Filter men's products from assets.products
  const mensProducts = products.filter((product) => product.category === "Men");

  // Get unique subcategories from products
  const subcategories = [
    ...new Set(mensProducts.map((product) => product.subCategory)),
  ];
      ...
      <section className="relative h-[400px]">
        ...
        <div className="absolute inset-0 bg-black bg-opacity-40 flex items-center justify-center">
          <div className="text-center text-white space-y-4">
            ...
            <p className="text-lg">Discover our latest trends and styles</p>
          </div>
        </div>
      </section>
      ...
      <section className="container mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {subcategories.map((subcategory) => {
            const firstProduct = mensProducts.find(
